const { verifyToken, verifyTokenAndAdmin, } = require("../middleware/verify-token")
const { Client, clientValidation , pageValidation , updateValidation,updateMembershipValidation}= require("../model/client")
//import bycrpt
...
router.get("/:id", async(req,res)=> {
    ...
    try{
        ...
        .then((docs)=> {
            ...
            res.status(500).json({
                status_code: -2,
                message: "There was an error when getting the client, please try again",
                data: null,
                error : {
                    message: error.message
                }
...
router.patch("/:id", async(req,res)=> {
    ...
    try{
        ...
        Client.findOneAndUpdate({ _id: req.params.id }, { $set: updateOps }, { new: true })
        .select('-__v')
        .then((docs)=> {
...
router.patch("/membership-status/:id", async(req,res)=> {
    ...
    try{
        ...
        Client.findOneAndUpdate({ _id: req.params.id }, { $set: {
